...
11. **Safari 26 stopped reading `<meta name="theme-color">` entirely.** It now tints its own UI (status bar, toolbar) from the *computed `background-color`* of `html`/`body` — falling back to white/black if that's transparent. `background: <gradient>` alone only ever sets `background-image`, leaving `background-color` at its transparent default, so the status bar rendered as a plain white bar over our warm gradient even with a correct theme-color tag in place. Fix: `background: <solid-color> <gradient>;` — the combined shorthand sets both. Confirmed via `getComputedStyle(el).backgroundColor` (was `rgba(0,0,0,0)`, should resolve to a real color). Keep the theme-color meta tag too — still correct for other browsers/older Safari, just not sufficient alone on Safari 26.
12. **Desktop Safari's Web Share API and `<a download>` are both unreliable, independently of each other.** `navigator.canShare({files})` can report `true` on Mac Safari while `navigator.share()` still rejects for the same file (works fine on iPhone Safari — this is desktop-specific). Separately, Safari's `download` attribute on a `blob:` URL can silently fall through to just navigating/opening the content instead of downloading it, on any platform. Never rely on either succeeding silently: catch `share()` rejections (excluding the user's own `AbortError` cancel) and fall back to a real download link, and put `target="_blank" rel="noopener"` on any `<a download>` so a fallen-through navigation opens in a new tab instead of losing the user's place.
13. **An element's `.textContent` persists across state changes unless explicitly cleared.** `#status` was set once by the recording countdown and never reset — invisible while it happened to render in an unused corner of the screen, but a real bug (stale "10s left" text bleeding into the next guest's idle screen) once a later change moved a different element into that same screen position. Toggling `display`/visibility is not the same as clearing content — anything that sets text conditionally needs an equally explicit reset path back to blank.
...
- **iPhone 14 Pro, iOS 26.5.2, Safari — confirmed 2026-07-23**: full guest flow works (countdown → record → stop → preview → upload), including with a staff frame/gesture asset set. Downloaded clip measured exactly 1080×1920 (the staff preset) in Photos. FaceLandmarker + HandLandmarker + beauty compositing + MediaRecorder running together stayed smooth even after the output canvas backing store grew from screen-sized (~219k px) to the full preset (~2.07M px, ~9.5×).
